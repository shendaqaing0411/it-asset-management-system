<template>
  <el-card>
    ...
    <el-dialog v-model="visible" :title="editing ? '编辑分类' : '新增分类'" width="400px">
...
async function fetch() { const r = await api.get('/categories'); items.value = r.data }
...
async function handleDelete(id) { await api.delete(`/categories/${id}`); ElMessage.success('已删除'); fetch() }
onMounted(() => fetch())
</script>
