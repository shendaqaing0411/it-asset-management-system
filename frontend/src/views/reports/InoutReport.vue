<template>
  <el-card>
    ...
    <el-form :inline="true" :model="query" size="small">
      ...
      <el-form-item label="类型"><el-select v-model="query.type" clearable style="width:140px"><el-option label="采购入库" value="采购入库" /><el-option label="领用出库" value="领用出库" /><el-option label="借用出库" value="借用出库" /><el-option label="归还" value="归还" /><el-option label="报废" value="报废" /></el-select></el-form-item>
...
const records = ref([])
const summary = ref([])
const query = ref({ start_date: '', end_date: '', type: null })

async function fetch() {
  const res = await api.get('/report/inout', { params: { ...query.value, type: query.value.type || undefined } })
