<template>
  <el-card>
    ...
    <el-form :inline="true" :model="query" size="small">
      ...
      <el-form-item label="一级分类"><el-select v-model="query.parent_category_id" clearable style="width:160px" @change="onParentCategoryChange"><el-option v-for="c in parentCategories" :key="c.id" :label="c.name" :value="c.id" /></el-select></el-form-item>
      <el-form-item label="二级分类"><el-select v-model="query.category_id" clearable style="width:160px"><el-option v-for="c in subCategories" :key="c.id" :label="c.name" :value="c.id" /></el-select></el-form-item>
...
const query = ref({ warehouse_id: null, category_id: null, parent_category_id: null, status: null })
const statusMap = { in_stock: '在库', in_use: '使用中', borrowed: '借出', repairing: '维修中', scrapped: '已报废' }

function onParentCategoryChange() {
  query.value.category_id = null
  if (query.value.parent_category_id) {
    const parent = categories.value.find(c => c.id === query.value.parent_category_id)
...
async function fetch() {
  loading.value = true
  try {
    const params = { ...query.value, page_size: 500 }
